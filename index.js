...
const time = moment.tz('Asia/Jakarta').format('HH:mm:ss')			const wita = moment.tz('Asia/Makassar').format('HH:mm:ss')			
...
nocache('./admin.js', modul => console.log(color('|ADMIN|'), color(`${modul} Updated!`, 'red')))
...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('E-BOTZ', {
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6289527031603@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `E-BOTZ`, img,but = [{buttonId:`apaaja`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `E-BOTZ`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `AMIN`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6289527031603@s.whatsapp.net","6289527031603@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]

        dtod = "6289527031603@s.whatsapp.net"
