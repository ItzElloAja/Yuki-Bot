...
const dtod = "6289527031603@s.whatsapp.net"
const otod = "6289527031603@s.whatsapp.net"
// STICKER WM
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			kurr.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title: `${jmn} - ${week}  - ${calender}`,body:"KurrXdゑ v.3.0",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6349482305092740/6289527031603`}}})
		}
...
kurr.sendMessage("6289527031603@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
...
cpref = `Hay Saya ElloWhatsapp Ketik .menu Untuk Mengakses`
reply(cpref)
}
if(budy.includes('assalamualaikum')){
cpref = `Hay Saya ElloWhatsapp Ketik .menu Untuk Mengakses`
...
if (isCmd && !isOwner) antiSpam.addFilter(from)
		...
		//runtime berjalan
           runi = process.uptime() 
           kurr.setStatus(`Hy I'm Ello🗿`).catch((_)=>_);
...
kurr.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]}})
 ...
 footerText: '*_© Ɇ-ɃØŦƵ 𒆜_*',
 description: `Hai @${stod.split('@')[0]}\nSaya Adalah Ɇ-ɃØŦƵ 𒆜`,
 sections: [
                     {
...
_書 ${prefix}othermenu_
 `
sendButLocation(from, `${menu}`, "*_© Ɇ-ɃØŦƵ 𒆜_*", {jpegThumbnail:ofrply2,name:""}, [{buttonId:`info`,buttonText:{displayText:'INFO BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
_書 ${prefix}nano [ _nama file_ ]_             
 `
sendButLocation(from, `${menu}`, "*_© Ɇ-ɃØŦƵ 𒆜_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
_kurrxdstore.xyz_
`
sendButLocation(from, `${menu}`, "*_© Ɇ-ɃØŦƵ 𒆜_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`store`,buttonText:{displayText:'STORE BOT'},type:1},{buttonId:`speed`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
case 'catlog': 
       ...
       ownerJid = "6289527031603@s.whatsapp.net"
                nomor = 1
                stod = `${sender}`
       stst = await kurr.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
...
ini_list.push({
"displayName": `Developer Ɇ-ɃØŦƵ 𒆜`,
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    kurr.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`Ɇ-ɃØŦƵ 𒆜`}, MessageType.location)
...
var nn = body.slice(9)
          ...
          if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY Ɇ-ɃØŦƵ 𒆜`)
...
break
			...
			case 'speed':
			case 'ping':
ini_txt = `Ɇ-ɃØŦƵ 𒆜 Merespon Dalam ${latensyi.toFixed(4)} DETIK
`
reply(ini_txt)
break
				case 'return':
...
break
 ...
 `
sendButLocation(from, `${menu}`, "*_© Ɇ-ɃØŦƵ 𒆜_*", {jpegThumbnail:ofrply3,name:""}, [{buttonId:`owner`,buttonText:{displayText:'OWNER THIS BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title: "Developer Ɇ-ɃØŦƵ 𒆜",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/6349482305092740/6289527031603"}}})
	}
    }
    }
    }
